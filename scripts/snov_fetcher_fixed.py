...
OUTPUT_FILE = Path("c:/Users/91639/Documents/antigravity/job-scout-engine/data/snov_contacts_fixed.json")
...
def fetch_domain_contacts(domain, token):
    ...
    if start_resp.status_code not in [200, 202, 201]:
        return {"domain": domain, "error": f"Failed to start: {start_resp.status_code} {start_resp.text}"}
